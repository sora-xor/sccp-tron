...
///
/// This contract is designed to be chain-agnostic across EVM chains and can be used as the
/// `ISccpVerifier` implementation for SCCP routers on Ethereum/BSC/TrON.
contract SoraBeefyLightClientVerifier is ISccpVerifier {
    ...
    function _bitIsSet(bytes memory bf, uint256 index) private pure returns (bool) {
        ...
        uint8 bitInByte = uint8(index & 7);
        uint8 mask = uint8(1 << (7 - bitInByte)); // Msb0
